...
bool IniFileParser::ParseVelocityModel(TiXmlElement* xVelocity, TiXmlElement* xMainNode)
{
     ...
     //force_ped
     if (xModelPara->FirstChild("force_ped")) {
          if (!xModelPara->FirstChildElement("force_ped")->Attribute("death_factor"))
               _config->SetDeathFactor(1.0); // default value
          else {
               string death = xModelPara->FirstChildElement("force_ped")->Attribute("death_factor");
               _config->SetDeathFactor(atof(death.c_str()));
               std::cout <<_config->GetDeathFactor() << "\n";
     ...
     _config->SetModel(std::shared_ptr<OperationalModel>(new VelocityModel(_exit_strategy,
                                                                           _config->GetaPed(),
                                                                           _config->GetDPed(),
                                                                           _config->GetaWall(),
                                                                           _config->GetDWall(),
                                                                           _config->GetDeathFactor()
                                                              )
                            )
                       );
