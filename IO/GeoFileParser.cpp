...
bool GeoFileParser::LoadGeometry(Building* building)
{
     ...
     if (version<std::stod(JPS_OLD_VERSION)) {
          ...
          Logging::Error(fmt::format("Please update the version of your geometry file to {:s}", JPS_OLD_VERSION));
          return false;
     }
...
bool GeoFileParser::LoadTrainTimetable(Building* building, TiXmlElement * xRootNode)
{
     ...
     if (xTTT->ValueStr() != "train_time_table") {
          Logging::Error("Parsing train timetable file. Root element value is not 'train_time_table'.");
          return false;
     }
